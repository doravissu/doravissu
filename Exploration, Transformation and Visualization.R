...
p+geom_point(aes(size=Gender,color=Income))+
  scale_size_discrete(labels=c("F","M"),range = c(1,3))+
  scale_color_continuous(low="blue",high="red")+
  geom_smooth()+facet_grid(Gender~Employment)
...
setwd("F:\\Jigsaw Academy\\Trainings\\Data Exploration and Visualization")
...
#qplot(LnSales, LnPrice)
#qplot(OfflineSpend, NewVolSales)
